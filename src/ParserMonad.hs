...
item2 :: Parser String
item2 = Parser $ \ input -> case input of
                             "" -> Nothing
                             (p:_) -> Nothing
                             ['-'] -> Nothing
                             ('-':p:_) -> Nothing
...
notNatParser :: Parser Float
notNatParser = do digits <- some digit
                  return $ read digits
...
floatParser = do r <- (literal "-") <||> notNatParser
                 case r of
                  Left _ -> fmap (0-)  notNatParser
                  Right n -> return n

-- parse bools
boolParser :: Parser Bool
boolParser = do r <- (literal "true") <||> (literal "false")
                case r of
                  Left _ -> return True
